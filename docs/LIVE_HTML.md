...
- `examples/live-html/chatroom.ts` uses a connection-scoped `@component()` backed by a shared room service. Visitors join once, receive a dedicated composer, see online members, share bounded history, and recover their identity and missed messages after reconnect.
