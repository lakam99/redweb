...
Redweb uses that same configured origin as `publicOrigin` for its HTTP CSRF boundary, so same-origin forms remain valid when TLS terminates at the proxy. Cross-site Fetch Metadata and foreign origins remain blocked.
