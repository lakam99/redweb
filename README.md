...
`npm run verify:evaluation:controls:coverage` checks the unchanged control validator separately: four working protocol controls and seven deliberately broken variants run in actual Chromium on Windows. Elsewhere, the native check requires a clean unsupported-interface refusal, not browser success. Explicit verifier-result units test reporting failures and immutable output files; unexpected native outcomes retain their original errors and workspace. These evaluator controls are not new Redweb agent submissions or release acceptance.
