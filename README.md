...
Run `npm run verify:browser:coverage` for native Chromium tests of the complete emitted Live HTML runtime, including DOM morphing, action feedback, form serialization and state/event wiring. The gate enforces 100% statement/branch/function/line coverage and runs the same cases without instrumentation. Actual HTTP/WebSocket checks cover action failures, selection preservation, native input reaching server-owned state, malformed patch reporting and no offline-state replay after reconnect. Ownership edge cases use native-DOM unit-style tests; browser and transport APIs are not replaced. This scope excludes the separately imported `redweb-client` package, development refresh and whole-application coverage; it is not an exhaustive browser-coverage claim.
