...
Run `npm run verify:browser:coverage` for native Chromium tests of the generated DOM-morph and action-feedback modules, enforcing 100% statement/branch/function/line coverage and testing identical behavior without instrumentation. It also verifies select preservation and action feedback through actual live pages' HTTP/WebSocket actions. Ownership edge cases use native-DOM unit-style tests; browser and transport APIs are not replaced. The surrounding transport/form runtime, development refresh and whole-application coverage remain separate work; this is not a complete browser-coverage claim.
