...
function summarize(log, root = path.resolve(__dirname, '../..')) {
    ...
    for (const line of lines.slice(1)) {
        ...
        if (!line.startsWith('code-creation,')) continue;
        ...
        assert(/^\d+$/.test(fields[3]) && Number.isSafeInteger(time) && time >= lastTime, 'Invalid code timestamp');
        ...
        lastTime = time;
        events.push({ kind: kind === -2 ? 'CALLBACK' : kinds[kind], time, bytes, ...location(fields[6], root) });
    }
    const boundary = name => {
    ...
    return { diagnosticOnly: true, retainedSizeProven: false, version: header,
        ...
        intervalCreationRecords: selected.length, rows };
}
