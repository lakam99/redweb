...
// Shared real-network acceptance for packed and authored-source measurements.
async function verifyRoomApplication(demo) {
    let peer, failure;
    try {
    ...
    // A broken peer must not prevent releasing the owned server. Preserve the
    // original verification failure while retaining every cleanup failure.
    for (const cleanup of [() => closeWebSocket(peer), () => demo.shutdown()]) {
        try { await cleanup(); }
        catch (error) {
            const cleanupError = verificationError(error);
            failure = failure ? new AggregateError([failure, cleanupError], failure.message, { cause: failure }) : cleanupError;
        }
    }
    if (failure) throw failure;
}

module.exports = { verifyRoomExample, verifyRoomApplication };
